...
async function runImport() {
  ...
  console.log(`Successfully parsed ${totalParsed} questions total. Inserting into database...`);

  // Fetch approved questions
  const approvedQuestions = new Set<string>();
  try {
    const [rows]: any = await pool.query("SELECT source_file, source_question_no FROM questions");
    for (const row of rows) {
      if (row.source_file && row.source_question_no !== undefined) {
        approvedQuestions.add(`${row.source_file}::${row.source_question_no}`);
      }
    }
    console.log(`Loaded ${approvedQuestions.size} approved questions from live database.`);
  } catch (err) {
    console.error("Error loading approved questions:", err);
  }

  // Fetch rejected questions before clearing pending table
  const rejectedQuestions = new Set<string>();
  try {
    const [rows]: any = await pool.query("SELECT source_file, source_question_no FROM review_pending_questions WHERE status = 'rejected'");
    for (const row of rows) {
      if (row.source_file && row.source_question_no !== undefined) {
        rejectedQuestions.add(`${row.source_file}::${row.source_question_no}`);
      }
    }
    console.log(`Loaded ${rejectedQuestions.size} rejected questions from review database.`);
  } catch (err) {
    console.error("Error loading rejected questions:", err);
  }

  // Clear existing review questions to allow clean run
  try {
    await pool.query("DELETE FROM review_pending_questions");
    console.log("Cleared existing review_pending_questions.");
  } catch (err) {
  ...
  for (const q of allQuestions) {
    try {
      ...
      if (approvedQuestions.has(key)) {
        status = 'approved';
      } else if (rejectedQuestions.has(key)) {
        status = 'rejected';
      }
  ...
  console.log(`Database insertion complete. Inserted ${insertedCount} / ${totalParsed} questions.`);
  process.exit(0);
}

runImport().catch(err => {
  console.error("Critical error during import run:", err);
  process.exit(1);
});
